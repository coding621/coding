...
import com.morpheus.coding.api.workflow.DeployFeignService;
...
import com.morpheus.coding.project.domain.*;
import com.morpheus.coding.project.domain.vo.project.ProjectVO;
import com.morpheus.coding.project.domain.vo.project.log.*;
...
import com.morpheus.coding.project.mapper.*;
import com.morpheus.coding.project.service.ProjectLogService;
import com.morpheus.coding.project.service.ProjectTaskService;
import com.morpheus.coding.project.service.task.QueryTaskLogFactory;
...
public class ProjectTaskServiceImpl extends ServiceImpl<ProjectTaskMapper, ProjectTask> implements ProjectTaskService {
    ...
    @Override
    @GlobalTransactional(name = "coding-project-addTask",rollbackFor = Exception.class) //seata分布式事务，AT模式
    public String add(TaskReqVO taskReqVO) {
        ...
        // 5、添加或更新审批设置（远程调用 coding-workflow 微服务）
...
//            // 设置任务详情地址
//            String url = SsoUrlUtils.ssoCreate(appid, agentid, host + path + ssoPath + URLEncoder.encode(host + "/coding-project/my-task/info?taskId=" + taskId));
...
//            taskAssignRemindDTO.setLinkUrl(OAUtils.ssoCreate(host + "/coding-project/my-task/info?taskId=" + taskId));
            // TODO: 2024.03.03  推送消息暂时关闭
...
//        }
    ...
    public void downloadTaskTemplate(HttpServletResponse response) throws IOException {
        String filePath = CodingConfig.getProfile() + "/template/taskTemplate.xlsx";
        response.setContentType(MediaType.APPLICATION_OCTET_STREAM_VALUE);
